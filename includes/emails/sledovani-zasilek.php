...
if ( ! empty( $id_zasilky ) && ! empty( $dopravce ) ) {
  ...
  $aria_label = $carrier_name . ' ' . $external_link_tip;
  $odkaz_html = '<a href="' . esc_url( $odkaz ) . '" target="_blank" rel="noopener noreferrer" aria-label="' . esc_attr( $aria_label ) . '">' . esc_html( $carrier_name ) . '</a>';
?>
<p>
  <?php printf( esc_html__( 'Objednávka byla odeslána a můžete ji sledovat na stránkách dopravce: %s.', 'ceske-sluzby' ), $odkaz_html ); ?>
</p>
<?php } ?>
