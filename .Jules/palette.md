...
## 2025-05-17 - [Email UX consistency and conditional tracking links]
**Learning:** Tracking links in emails can lead to broken UX if data is missing. Wrapping them in conditional blocks ensures only complete information is sent. Furthermore, using `sprintf()` for localized links allows for better accessibility (adding `aria-label`) and security (`rel="noopener noreferrer"`) while maintaining translatability. Consistency between HTML and plain-text versions is crucial for a unified user experience.
**Action:** Always wrap tracking sections in `if ( ! empty(...) )` blocks in email templates. Use descriptive link labels (e.g., carrier name) instead of generic "here" text. Ensure the same data retrieval logic and localized strings are used across all email formats.
